// blant-compat.c: Implementation of compatibility functions replacing libwayne/src/misc.c
// This provides the same API as libwayne using standard C only.
#include "blant-compat.h"
#include <sys/mman.h>
#include <sys/time.h>
...
// Try to mmap, and if it fails, just slurp in the file
...
long long IIntPow(int a, int n)
{
    ...
    assert(n >= 0);
    if(n == 1) return a;
    if(n == 0) return a == 0 ? NAN : 1;
    result = IIntPow(a, n/2);
    if(n & 1) return a * result * result;
...
double IntPow(double a, int n)
{
    double result;
    if(n == 1) return a;
    if(n == 0) return a == 0.0 ? NAN : 1;
    if(n < 0) return 1/IntPow(a,-n);
    result = IntPow(a, n/2);
    if(n & 1) return a * result * result;
    else return result * result;
}
...
{
    double absX=fabs(x);
    if(absX<1e-16) return x;
    if(absX>4e-6) return log(1+x);
    assert(x>=-0.5 && x<=1);
#define MAX_LOG_TERMS 1000
    double sum=0, sumNeg=0, sumPos=0, term=x, _log1Terms[MAX_LOG_TERMS];
    int i, n=1;
...
int PrimeFactors(int N, int count[])
{
    ...
    for(p=0; p<=s; p++) {
        count[p] = 0;
        if(IsPrime(p)) {
            while(n%p==0) { prod *= p; total++; count[p]++; n /= p; }
        }
    }
    assert(prod == N);
    return total;
...
const char* getDecompressionProgram(char* fileName) {
    ...
    if(strcmp(ext, "gz") == 0) return "gunzip";
    else if(strcmp(ext, "xz") == 0) return "unxz";
    else if(strcmp(ext, "bz2") == 0) return "bzip2 -dk";
    return "";
}
...
FILE* decompressFile(const char* decompProg, char* fileName) {
    ...
    fprintf(stderr, "Decompressing file %s using decompression program %s\n", fileName, decompProg);
    return popen(result, "r");
}

void closeFile(FILE* fp, int* isPiped)
{
    if((*isPiped) == 1) pclose(fp);
    else fclose(fp);
}
