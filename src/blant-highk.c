...
void HighK_Init(int k)
{
    ...
    /* For k>8, try to load canon_list file (generated by nauty-based tools).
     * For k<=8, the baseline canon_list files use a different canonical form
     * (brute-force permutation search vs nauty), so we always use on-the-fly
     * mode for k<=8 in the HIGH_K binary.
     */
    if (k > 8) {
    ...
    /* On-the-fly mode: nauty per sample, ordinals assigned dynamically */
    _highK_mode = 2;
